...
out_busco = apply(data1,1,function(x){   print(x)
  busco_tab = read.delim(paste("database/BUSCO_annotations/",x["species"],"_NCBI.taxid",as.numeric(x["NCBI.taxid"]),"/",x["assembly_accession"],"/busco_to_gene_id_eukaryota",sep=""))
