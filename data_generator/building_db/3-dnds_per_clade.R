...
compute_files <- function(name){
  ...
  write.tree(tree,paste("data/dnds_phylo/",name,"_dN.nwk",sep=""))
  
  tree$edge.length = dt_per_edge$dS / dt_per_edge$dS_norm
  write.tree(tree,paste("data/dnds_phylo/",name,"_dS.nwk",sep=""))
  
  data = data.frame(species = tree$tip.label,
                    NCBI.taxid = list_species[tree$tip.label,]$NCBI.taxid,
                    dN = get_newick_value(read.tree(paste("data/dnds_phylo/",name,"_dN.nwk",sep="")))[tree$tip.label],
                    dS = get_newick_value(read.tree(paste("data/dnds_phylo/",name,"_dS.nwk",sep="")))[tree$tip.label]
  )
  data$dNdS = data$dN / data$dS
  
  write.table(data , paste("data/dnds_phylo/",name,".tab",sep=""),quote=F,row.names = F,sep="\t")
}
