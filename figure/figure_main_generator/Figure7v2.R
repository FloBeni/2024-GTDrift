...
pA = ggplot(dt_graph , aes_string(x=xlabel,y=ylabel,fill="clade_group")) +
  ...
  scale_x_log10(breaks=c(10^-6,10^-4,10^-2,10^0,10^2,10^4,10^6),labels=label_log(digits = 2),limits = c(0.0001,10000)) + xlab("Body Weight (kg, log scale)")+
...
pC = ggplot(dt_graph , aes_string(x=xlabel,y=ylabel,fill="clade_group")) +
  ...
  ylab("Terminal branches dN/dS per clade set") + annotation_logticks(sides = "b") + scale_y_log10()
pC
