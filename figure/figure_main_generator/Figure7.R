...
jpeg(paste(path_pannel,"F7pA.jpg",sep=""),width = 7000/resolution, height = 4000/resolution,res=700/resolution)
print(pA)
dev.off()
...
pB = ggplot(dt_graph , aes_string(x=xlabel,y=ylabel,fill="clade_group")) + geom_point(pch=21,size=3,alpha=.6)  + 
  ...
  ylab("Terminal branches dN/dS per clade set") +  theme(legend.position="none")+ annotation_logticks(sides = "b")
pB
...
jpeg(paste(path_pannel,"F7pC.jpg",sep=""),width = 5200/resolution, height = 4000/resolution,res=700/resolution)
print(pC)
dev.off()
...
{
  pdf(file= paste(path_figure,"Figure7.pdf",sep=""), width=3*5/2, height=2.75*3*1.33)
