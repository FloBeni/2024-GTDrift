...
jpeg(paste(path_pannel,"F4pA.jpg",sep=""),width = 8500/resolution, height = 4000/resolution,res=700/resolution)
print(pA)
dev.off()
...
pB=ggplot(data1 , aes(x=max_weight_kg,y=max_length_cm,fill=clade_group)) + geom_point(pch=21,size=3,alpha=.8)  + 
  scale_fill_manual("Clades",values = Clade_color ) + theme_bw() + theme(
    ...
    plot.caption = element_text(hjust = 0.4, face= "italic", size=23, family="economica"),
    plot.caption.position =  "plot"
  ) + guides(fill = guide_legend(override.aes = list(size=5))) +  ggtitle(paste("Nspecies=",nrow(data1[!is.na(data1$max_length_cm) & !is.na(data1$max_weight_kg),]),sep=""))+
...
jpeg(paste(path_pannel,"F4pB.jpg",sep=""),width = 6800/resolution, height = 4000/resolution,res=700/resolution)
print(pB)
dev.off()
...
imgA = load.image(paste(path_pannel,"F4pA.jpg",sep=""))
imgB = load.image(paste(path_pannel,"F4pB.jpg",sep=""))


{
  pdf(file= paste(path_figure,"Figure4.pdf",sep=""), width=4*5/2, height=2.75*3)
