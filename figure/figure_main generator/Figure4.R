...
p1 = ggplot(data1 , aes(y = nb_busco_gene_eukaryota/303 * 100 , fill = clade_group, x = clade_group)) + 
...
jpeg(paste(path_pannel,"F4p1.jpg",sep=""),width = 8500/resolution, height = 4000/resolution,res=700/resolution)
print(p1)
dev.off()
...
imgA = load.image(paste(path_pannel,"F4p1.jpg",sep=""))


{
  pdf(file= paste(path_figure,"Figure4.pdf",sep=""), width=6*5/2, height=2.75*3)
  ...
  par(mar=c(0, 2, 0, 0))
  plot(imgA, axes=F)
  mtext("A",at=80,adj=0, side=2, line=1, font=2, cex=3,las=2)
