...
max_clade = tapply(species_clade$value,paste(species_clade$species,species_clade$lht),max) # Keep the maximum per species and per life history traits.
...
max_lht$db = tapply(species_clade$db,paste(species_clade$species , species_clade$lht) , function(x) paste(x,collapse = ";"))[paste(max_lht$species,max_lht$lht)] # Get all the db that refer this maximum value.
